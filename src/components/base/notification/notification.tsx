...
function NotificationAvatarVisual({ presence, className, ...avatar }: NotificationAvatar) {
  return (
    <span className="relative shrink-0">
      <Avatar {...avatar} size="lg" className={cx('size-10', className)} />
      {presence ? (
        <span
          aria-hidden
          className={cx(
            'border-background-primary-default absolute right-0 bottom-0 size-3 rounded-full border-2',
...
export function Notification({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => setDismissed(true), autoDismissDuration + introDelayMs)
